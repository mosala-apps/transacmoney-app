    ...
    >
      ...
      <template #[`item.status`]="{ item }">
        <v-chip size="small" :color="getTransactionStatusColor(item.selectable?.status)">
          <span class="text-blue-500">
            {{ getFormattedTransactionStatus(item.selectable?.status) }}</span
          >
        </v-chip>
      </template>
      <template #[`item.actions`]="{ item }">
        ...
        <v-btn
          variant="outlined"
          color="blue"
          @click="router.push(`/transactions/deposit/${item.selectable.transactionCode}`)"
          >Details</v-btn
        >
      </template>
    </v-data-table>
...
import {
  getDateFormatted,
  getFormattedTransactionStatus,
  getFormattedTransactionType,
